...
public class TowerHpBar : MonoBehaviour
{
   ...
   //受到攻击后多久血条自动消失
   public float hpBarVisibleTime=3f;
   //是否一直显示血条
   ...
   private void OnHealthChanged(int currHealth,int maxHealth,int sub)
   {
      Debug.Log("sub"+sub);
      if (currHealth <= 0)
      {
      ...
      if (sub > 0)
      {
         demageTrans.GetComponent<DemageText>().setText(Constant.TEXT_RECOVE,sub.ToString());
      }
      else
      {
         demageTrans.GetComponent<DemageText>().setText(Constant.TEXT_DAMAGETOWER,Math.Abs(sub).ToString());
      }
   ...
   public void showStringText(string text)
   {
      demageTrans=Instantiate(demageText, canvasTrans).transform;
      demageText.GetComponent<DemageText>().setText(Constant.TEXT_STRING,text);
      demageTrans.position = demageTextPoint.position;
   }
   
   public void showStringText(string text,Color color)
   {
      demageTrans=Instantiate(demageText, canvasTrans).transform;
      demageText.GetComponent<DemageText>().setText(Constant.TEXT_STRING,text,color);
      demageTrans.position = demageTextPoint.position;
   }
